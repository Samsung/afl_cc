			...
			do
				...
				if [ $USE_MASTER -ne 0 ]; then
					...
					if [ "$DICTFILE" != "none" ] && [ $master_done -eq 0 ]; then
						master_done=1
						cmd="timeout ${TIME}s $AFLFUZZ -m none -i $INFOLDER -o $OUT -M M-$B -x $DICTFILE $b $ARGS @@"
					else
						cmd="timeout ${TIME}s $AFLFUZZ -m none -i $INFOLDER -o $OUT -S S-$B $b $ARGS @@"
					fi
				else
					if [ "$DICTFILE" != "none" ]; then
						cmd="timeout ${TIME}s $AFLFUZZ -m none -i $INFOLDER -o $OUT -S $B -x $DICTFILE $b $ARGS @@"
					else
						cmd="timeout ${TIME}s $AFLFUZZ -m none -i $INFOLDER -o $OUT -S $B $b $ARGS @@"
					fi
				fi
