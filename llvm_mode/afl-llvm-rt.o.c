...
u32 __afl_area_size = 0;
u32 __afl_bbtrace_size = 0;
u8 * __afl_area_initial = 0;
u8 * __afl_area_ptr = 0;
u8 * __afl_bbtrace_initial = 0;
u8 * __afl_bbtrace_ptr = 0;

static u8 bb_trace = 0;

__thread u32 __afl_prev_loc;

/* Get size of area */

extern u32 __afl_get_area_size(void);
extern u32 __afl_get_bbarea_size(void);
...
static void __afl_release_area(void) {
  free(__afl_area_initial);
  __afl_area_initial = __afl_area_ptr = 0;
  __afl_area_size = 0;
}
...
static void __afl_init_bbtrace(void) {
  ...
  /* Now get the number of bytes needed: TODO */
  __afl_bbtrace_initial = malloc(__afl_area_size); assert (__afl_bbtrace_initial);
  __afl_bbtrace_ptr = __afl_bbtrace_initial;
}

static void __afl_release_bbtrace(void) {
  free(__afl_bbtrace_initial);
  __afl_bbtrace_initial = __afl_bbtrace_ptr = 0;
}
...
static void __afl_map_bbtrace_shm(void) {

  /* Setup the shared mem region for bbtrace coverage */
  u8 *id_str_cov = getenv(SHM_ENV_BBTRACE_VAR);
 
  if (id_str_cov) {

    u32 shm_id_cov = atoi(id_str_cov);

    __afl_bbtrace_ptr = shmat(shm_id_cov, NULL, 0);

    /* Whooooops. */
    if (__afl_bbtrace_ptr == (void *)-1) _exit(1);
...
/* bbtrace tracing */
void __afl_bb_trace(u32 bb_id) {
  if (bb_trace) {
    set_bit_from_bb_id(__afl_bbtrace_ptr, __afl_bbtrace_size, bb_id);
  }
}
...
void __afl_manual_release(void) {
  if (init_done) {
    __afl_unmap_bbtrace_shm();
    __afl_unmap_shm();
    __afl_release_bbtrace();
    __afl_release_area();
    init_done = 0;
  }
}
...
/* Proper release routine. */

__attribute__((destructor(CONST_PRIO))) void __afl_auto_release(void) {
  __afl_manual_release();
}
...
void __sanitizer_cov_trace_pc_guard_init(uint32_t* start, uint32_t* stop) {
  ...
  *(start++) = R(__afl_area_size - 1) + 1;

  while (start < stop) {

    if (R(100) < inst_ratio) *start = R(__afl_area_size - 1) + 1;
    else *start = 0;
