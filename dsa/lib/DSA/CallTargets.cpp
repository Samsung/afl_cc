...
void CallTargetFinder<dsa>::print(llvm::raw_ostream &O, const Module *M) const
{
  ...
  template<class dsa>
void CallTargetFinder<dsa>::saveTargets(const Module *M) const
{
  ...
  for (std::map<CallSite, std::vector<const Function*> >::const_iterator ii =
    ...
    if(isa<Function>(ii->first.getCalledValue()->stripPointerCasts()))
      ...
      if (!isComplete(ii->first)) {
        for (std::vector<const Function*>::const_iterator i = ii->second.begin(),
               e = ii->second.end(); i != e; ++i) {
          FunctionSet.insert((*i)->getName().str());
        }
      }
      
      // ASSERT(isComplete(ii->first));
  ...
  std::string FuncStr;
  for (auto & s: FunctionSet) {
    if (FuncStr.size()) FuncStr += " " + s;
    else FuncStr = s;
  }

  LLVMContext & C = M->getContext();
  MDString * S = MDString::get(C, FuncStr); ASSERT (S);
  MDNode* Meta = MDNode::get(C, S);
  Function * Main = M->getFunction("main"); ASSERT(Main);
  Main->setMetadata("FuncPointedToList", Meta);
  
}

  template<class dsa>
bool CallTargetFinder<dsa>::runOnModule(Module &M) {
  getGlobalContext().getMDKindID("FuncPointedToList");
  findIndTargets(M);
  //print(errs(), &M);
  saveTargets(&M);
  return false;
